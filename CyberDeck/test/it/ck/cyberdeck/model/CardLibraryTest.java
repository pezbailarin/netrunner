...
public class CardLibraryTest {
	...
	private static final String TEST_CARD_LIBRARY = "bin/raw/carddata.js";
	...
	public void iCanGetTheCardsGroupedByTypePerIdentity() throws Exception {
		Identity identity = new Identity("name", Side.CORP,
				Faction.HAAS_BIOROID, 45, 15, null);
	...
	@Test
	public void iCanGetTheCardsGroupedByTypePerIdentityIdentitiesExcluded()
			throws Exception {
		Identity identity = new Identity("name", Side.CORP,
				Faction.HAAS_BIOROID, 45, 15, null);
		List<ElementGroup<Card>> list = cl
				.getCardGroupsWithoutIdentities(identity);
		assertThat(list, is(not(nullValue())));
		assertThat(list.size() > 0, is(true));
